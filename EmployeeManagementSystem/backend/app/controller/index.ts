...
import { createAccount, loginUser , isUserExist , getUserRole} from "../repository";
...
async function HandelCreateUserAccount(req : Request , res : Response , next : NextFunction){
    ...
    try{
        ...
        const getUserRoleRes = await getUserRole(userId)

        if(getUserRoleRes?.Role != "ADMIN"){
            throw new CustomError("Unauthorized , only admin can make new accounts" , 400)
        }

        checkValidation(req)

        const { userName, userEmail, userPassword , Role }: { userName: string, userEmail: string, userPassword: string , Role : string } = req.body;
        ...
        const createAccountRes = await createAccount(userName , userEmail , hashedPassword , Role)
       ...
       // const tokenToSend = jwt.sign( dataTOMakeToken , SECRET_KEY ,{expiresIn : '10m'})
        ...
        defaultRes(res , 200 , "account created successfully" , dataTOSend)       
...
async function HandelUserLogin(req : Request , res : Response , next : NextFunction){
    ...
    try{
        ...
        const tokenToSend = jwt.sign( dataTOMakeToken , SECRET_KEY)

        defaultRes(res , 200 , "account logged in , token generated" , {token : tokenToSend , role : isUserExistRes.Role })
